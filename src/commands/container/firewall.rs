...
pub async fn rules(
    ...
    if rules.is_empty() {
        out.print_message(&format!("No firewall rules for container {vmid}."));
        return Ok(());
    }
...
pub async fn add(
    ...
    out.print_result(
        ...
        &format!("Firewall rule added to container {vmid}"),
    );
    Ok(())
}
...
pub async fn delete(
    ...
    let node = client.resolve_node_for_vmid(vmid, node_override).await?;
    let path = format!("/nodes/{node}/lxc/{vmid}/firewall/rules/{pos}");

    let _: serde_json::Value = client.delete(&path).await?;

    out.print_result(
        &json!({"status": "deleted", "vmid": vmid, "pos": pos}),
        &format!("Firewall rule at position {pos} deleted from container {vmid}"),
    );
    Ok(())
}
