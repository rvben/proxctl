...
pub async fn clone(
    ...
    if let Some(h) = hostname {
        params.push(("hostname".to_string(), h.to_string()));
    }
    ...
    let param_refs: Vec<(&str, &str)> = params
        ...
        .collect();
    let path = format!("/nodes/{node}/lxc/{vmid}/clone");
    ...
    out.print_result(
        ...
        &format!("Container {vmid} cloned to {new_vmid}"),
    );
    Ok(())
}
...
pub async fn migrate(
    ...
    let node = client.resolve_node_for_vmid(vmid, node_override).await?;
    let path = format!("/nodes/{node}/lxc/{vmid}/migrate");
    ...
    out.print_result(
        ...
        &format!("Container {vmid} migrated to {target}"),
    );
    Ok(())
}
...
pub async fn template(
    ...
    let node = client.resolve_node_for_vmid(vmid, node_override).await?;
    let path = format!("/nodes/{node}/lxc/{vmid}/template");

    let _: serde_json::Value = client.post(&path, &[]).await?;

    out.print_result(
        &json!({"status": "converted", "vmid": vmid}),
        &format!("Container {vmid} converted to template"),
    );
    Ok(())
}
