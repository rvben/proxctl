"""
Command-line interface for proxctl.
"""
...
def find_native_binary() -> str:
    ...
    target_binary = project_root / "target" / "release" / "proxctl"
    if target_binary.exists() and not target_binary.is_dir():
        return str(target_binary)

    if sys.platform == "win32":
        target_binary = project_root / "target" / "release" / "proxctl.exe"
        if target_binary.exists() and not target_binary.is_dir():
            return str(target_binary)

    raise FileNotFoundError(
        "Could not find the native proxctl binary. "
        "Please ensure it was built with 'cargo build --release'."
    )


def main() -> int:
    """Run the proxctl command line tool."""
    try:
